...
| Функция | Что делает |
|--|--|
| `validateSurvey()` | проверка перед волной: дубли ID и токенов, включённый вопрос без строки в `Answers`, неверный `Номер правильного ответа`, битые ограничения, сотрудник без токена, строки `Usages` с чужим ID, нечитаемый `min_answers` |
...
- `Employees` — только заголовки и тестовый профиль; реальных сотрудников и токенов нет;
...
- `Settings` — рабочие `wave` и `min_answers`, промпт-заглушка.
